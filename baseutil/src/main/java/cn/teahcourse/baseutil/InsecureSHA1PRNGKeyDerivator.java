...
public class InsecureSHA1PRNGKeyDerivator {
    ...
    //                index, that the exact value is not defined in STANDARD
    ...
    protected synchronized void nextBytes(byte[] bytes) {
        ...
        if (state == UNDEFINED) {
            ...
            //                        index, that beginning extra bytes are not more than 8,
    ...
    private static void updateHash(int[] intArray, byte[] byteInput, int fromByte, int toByte) {
        ...
        // the buffer's note is in the intArray[BYTES_OFFSET] element
