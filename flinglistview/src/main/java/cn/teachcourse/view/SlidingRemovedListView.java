...
public class SlidingRemovedListView extends ListView {
    ...
    private SlidingRemovedItem willFlingView;//要滑动的列表项 View
    private int position = INVALID_POSITION;//要滑动的列表项 View 的索引位置
    private int touchSlop;//最小滑动距离
    private OnItemRemovedListener onItemRemovedListener;

    public SlidingRemovedListView(Context context) {
        this(context, null);
    }

    public SlidingRemovedListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlidingRemovedListView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (position != INVALID_POSITION) {
                    ...
                    willFlingView = (SlidingRemovedItem) getChildAt(visibleIndex);
    ...
    private void restoreListItems() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child instanceof SlidingRemovedItem) {
                SlidingRemovedItem childLayout = (SlidingRemovedItem) child;
                if (willFlingView != childLayout)
                    childLayout.scrollTo(childLayout.getDeleteButton().getWidth(), 0);
            }
        }
    }
     ...
     * @param extendLayout
     */
    private void doDelete(SlidingRemovedItem extendLayout) {
