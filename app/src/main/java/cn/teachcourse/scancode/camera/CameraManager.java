...
package cn.teachcourse.scancode.camera;
...
public final class CameraManager {
	...
	private   int MAX_FRAME_WIDTH = 520;
	private   int MAX_FRAME_HEIGHT = 520;
	...
	public CameraManager(Context context) {
		...
		MAX_FRAME_WIDTH = context.getResources().getDimensionPixelSize(R.dimen.big_f_width);
		MAX_FRAME_HEIGHT = context.getResources().getDimensionPixelSize(R.dimen.big_f_height);
	...
	public boolean TheLightIsOnOrOff(){
		if (camera == null) {
			return false;//没有开
		}else{
	...
	/**
	 * Calculates the framing rect which the UI should draw to prepare the user
...
//			framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
	...
	public void openF(){
		...
		// TODO Auto-generated method stub
				if (camera == null) {
					Toast.makeText(context, "Camera not found", Toast.LENGTH_LONG);
...
//					button.setBackgroundColor(COLOR_WHITE);
				...
				if (!Parameters.FLASH_MODE_TORCH.equals(flashMode)) {
					...
					if (flashModes.contains(Parameters.FLASH_MODE_TORCH)) {
						...
						Toast.makeText(context, "Flash mode (torch) not supported",
								Toast.LENGTH_LONG);
						// Use the screen as a flashlight (next best thing)
